...
void main() {
  ...
  group('Nozaki Global Timer Tests (N01 - N15)', () {
    ...
    test(
      'N03: Port refresh at or after 15 minutes with no Nozaki resets timer to start fresh cycle',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        service.nozakiTimer.reset(startTime);
    ...
    test(
      'N04: Base Nozaki fails only due to target fatigue retains READY without resetting',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        service.nozakiTimer.reset(startTime);
        ...
        expect(service.nozakiTimer.anchorAt, startTime);
        expect(
          service.nozakiTimer.elapsed(portTime),
    ...
    // N05: 野埼改疲劳恢复
    test(
      'N05: Nozaki Kai sparkle success resets timer to start next cycle',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        service.nozakiTimer.reset(startTime);
    ...
    test(
      'N08: Already sparkling companions do not prevent port timer reset',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        service.nozakiTimer.reset(startTime);
    ...
    // N09: 普通编成修改其他船
    test(
      'N09: Manual hensei swap on companions with Nozaki in slot 1 resets timer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
    ...
    test(
      'N10: Moving Nozaki from slot 1 to slot 3 leaves 1/2 without Nozaki and does NOT reset timer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        final changeTime = DateTime.utc(2026, 8, 20, 10, 5);
          ...
          fleets: const <Fleet>[
            Fleet(
              ...
              shipIds: <int>[
                ...
                5,
                6,
              ], // Nozaki (1) moved to index 2 (slot 3)
              slotCount: 6,
            ),
          ],
        );
    ...
    // N11: 把 3 号野埼移到 2 号
    test(
      'N11: Moving Nozaki from slot 3 to slot 2 puts Nozaki into work position and resets timer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        final changeTime = DateTime.utc(2026, 8, 20, 10, 5);
          ...
          fleets: const <Fleet>[
            Fleet(
              ...
              shipIds: <int>[
                ...
                5,
                6,
              ], // Nozaki (3) moved to index 1 (slot 2)
              slotCount: 6,
            ),
          ],
        );
    ...
    // N12: Preset 出现野埼
    test(
      'N12: Preset select resulting in Nozaki in slot 2 does NOT reset Nozaki timer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        ...
        expect(service.nozakiTimer.anchorAt, startTime);
        expect(
          service.nozakiTimer.elapsed(presetTime),
    ...
    // N14: 双野埼共享
    test(
      'N14: Multiple fleets with Nozaki share a single global NozakiTimer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
  ...
  group('Akashi Global Timer Tests (A01 - A11)', () {
    ...
    // A02: 明石旗舰换随伴
    test(
      'A02: Swapping companions in an Akashi-flagship fleet resets AkashiTimer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        final changeTime = DateTime.utc(2026, 8, 20, 10, 5);
    ...
    test(
      'A03: Moving Akashi out of flagship does NOT reset AkashiTimer by that rule',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        final changeTime = DateTime.utc(2026, 8, 20, 10, 5);
    ...
    // A05: Preset 展开明石
    test(
      'A05: Expanding Akashi fleet via preset does NOT reset AkashiTimer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        ...
        expect(service.akashiTimer.anchorAt, startTime);
        expect(
          service.akashiTimer.elapsed(presetTime),
    ...
    test('A06: Port refresh before 20 minutes does not reset AkashiTimer', () {
      ...
      expect(service.akashiTimer.anchorAt, startTime);
      expect(
        service.akashiTimer.elapsed(portTime),
        const Duration(minutes: 12),
      );
    });
    ...
    // A07: 20分钟以后母港
    test(
      'A07: Port refresh at or after 20 minutes evaluates repair and restarts timer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
        service.akashiTimer.reset(startTime);
    ...
    // A08: 改装备
    test(
      'A08: Changing repair facility equipment does NOT reset AkashiTimer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
    ...
    test('A09: Sortie start and battle events do NOT reset AkashiTimer', () {
      ...
      expect(service.akashiTimer.anchorAt, startTime);
      expect(
        service.akashiTimer.elapsed(sortieTime),
    ...
    // A10: 跨舰队抽随伴
    test(
      'A10: Pulling companion from Fleet 1 (Akashi) into Fleet 2 modifies Fleet 1 and resets AkashiTimer',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
  ...
  group('Combined Akashi + Nozaki Tests (C01 - C04)', () {
    // C01: 同舰队明石+野埼
    test(
      'C01: Akashi slot 1 and Nozaki slot 2 in same fleet co-exist independently',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
    ...
    // C02: 15分钟节点
    test(
      'C02: At 15-minute port refresh, Nozaki sparkles & restarts while Akashi continues',
      () {
        final service = TimerMechanicsService();
    ...
    // C03: 20分钟节点
    test(
      'C03: At 20-minute port refresh, Akashi repairs & restarts while Nozaki continues',
      () {
        final service = TimerMechanicsService();
        final t1000 = DateTime.utc(2026, 8, 20, 10, 0);
    ...
    // C04: 同一普通编成同时命中两边
    test(
      'C04: Single manual change on Akashi+Nozaki fleet triggers resets for both timers',
      () {
        final service = TimerMechanicsService();
        final startTime = DateTime.utc(2026, 8, 20, 10, 0);
