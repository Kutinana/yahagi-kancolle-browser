...
void main() {
  test('projects unowned ship families and equipment master data', () {
    ...
    expect(projection.unownedShipFamilies.single.typeName, '驱逐舰');
    expect(projection.unownedEquipment.map((row) => row.master.id), <int>[102]);
    expect(projection.unownedEquipment.single.typeName, '小口径主炮');
  });
