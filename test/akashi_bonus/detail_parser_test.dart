...
Document fixtureDoc(String name) => html_parser.parse(
  File('test/akashi_bonus/fixtures/$name').readAsStringSync(),
);
...
void main() {
  group('parseDetailDocument', () {
    test('parses w266 identity, base stats, fits and tipbody', () {
      ...
      expect(
        r.tipGroups.map((g) => g.label),
        containsAll(['陽炎型改二', '陽炎型', '白露型', '朝潮型']),
      );
    ...
    test('fit 1 has base block, synergy block and target', () {
      ...
      expect(base.statBonuses.map((b) => b.rawText), [
        '火力+2,+3,+1',
        '回避+1(雪風改二のみ)',
      ]);
    ...
    test('w330 style: spans without sm1 class, no synergy', () {
      final html =
          '<html><body><div class="name jpfont"><span class="no">No.330 </span>'
    ...
    test('w122 style: divs with rstar and icons inside span', () {
      final html =
          '<html><body><div class="name jpfont"><span class="no">No.122 </span>'
    ...
    test('rbonus star table is captured', () {
      final html =
          '<html><body><div class="name jpfont"><span class="no">No.999 </span>'
    ...
    test('base stats ignore <r> remodel values', () {
      final html =
          '<html><body><div class="name jpfont"><span class="no">No.1 </span>'
    ...
    test('missing name throws', () {
      expect(
        () => parseDetailDocument(
          html_parser.parse('<html><body></body></html>'),
          1,
        ),
    ...
    test('malformed fit text throws', () {
      expect(
        () => parseDetailDocument(fixtureDoc('malformed_fit.html'), 306),
        throwsA(isA<DetailParseException>()),
      );
    });

    test('no bonus contents yields empty fits (no_bonus)', () {
      final r = parseDetailDocument(
        fixtureDoc('missing_bonus_contents.html'),
        308,
      );
