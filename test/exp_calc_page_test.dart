...
void main() {
  testWidgets('renders ExpCalcPage with initial empty state and default fields', (
    tester,
  ) async {
    final store = FakeExpTrackerStore();
    const state = GameState(memberId: 1);

    await tester.pumpWidget(_testableApp(ExpCalcPage(state: state, store: store)));
    ...
    expect(find.byKey(const Key('exp-calc-target-level-decrease')), findsOneWidget);
    expect(find.byKey(const Key('exp-calc-target-level-increase')), findsOneWidget);
    ...
    expect(find.byKey(const Key('exp-calc-flagship-checkbox')), findsOneWidget);
  ...
  testWidgets('selects ship and auto-populates levels, exp, and next remodel target', (
    tester,
  ) async {
    final store = FakeExpTrackerStore();
    ...
    expect(find.text('18600000 EXP'), findsOneWidget); // Target level 187 base exp
  ...
  testWidgets('supports multi-node route addition, deletion, and combined experience calculation', (
    tester,
  ) async {
    final store = FakeExpTrackerStore();
    final state = GameState(
      ...
      ships: {
        1: const OwnedShip(id: 1, masterId: 546, level: 186, experience: 17635281),
      },
    );
  ...
  testWidgets('adds track item with route summary, dynamically updates upon level-up, and deletes', (
    tester,
  ) async {
    final store = FakeExpTrackerStore();
    final state = GameState(
    ...
    await tester.pumpWidget(_testableApp(ExpCalcPage(state: updatedState, store: store)));
    await tester.pumpAndSettle();
    ...
    // Delete track item
    final deleteButtonKey = Key('exp-calc-delete-button-${store.items.first.id}');
  ...
  testWidgets('renders cleanly without overflow in narrow mode (360px width) and toggles view modes', (
    tester,
  ) async {
    final store = FakeExpTrackerStore();
    final state = GameState(
